...
def scrape_one(session: requests.Session, locality_id: int, outdir: Path, delay_s: float) -> Optional[str]:
    ...
    if content_path.exists() and not getattr(scrape_one, "overwrite", False):
        return None
        ...
        try:
            ...
            for img_url in fotoarchiv_candidate_image_urls(photo_id):
                ...
                if dest.suffix.lower() in {".jpg", ".jpeg"}:
                    exif_set_user_comment_jpeg(dest, pref.caption)
                    try:
                        normalize_utf8_jpeg_text(dest)
                        rewrite_jpeg_with_exif_utf8(dest)
                    except Exception:
                        pass
                break
        except Exception as e:
